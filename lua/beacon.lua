...
-- ensure get buffer. fix bug when buffer is deleted
local function get_fake_buffer()
    if vim.api.nvim_buf_is_valid(fake_buffer) then
        return fake_buffer
    else
        fake_buffer = vim.api.nvim_create_buf(false, true)
        return fake_buffer
    end
end
...
local function create_window(cfg)
    local window = vim.api.nvim_open_win(get_fake_buffer(), false, {
        relative = 'cursor',
        row = 0,
        col = 0,
        width = cfg.width,
